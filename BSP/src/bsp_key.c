...
static uint8_t IsKeyUpPress(void) { return gpio_get(KEY_UP_PIN);}
static uint8_t IsKeyOkPress(void) { return gpio_get(KEY_OK_PIN);}
static uint8_t IsKeyDownPress(void) { return gpio_get(KEY_DOWN_PIN);}
...
{
	gpio_init(KEY_UP_PIN, GPI, 1);
	port_pull(KEY_UP_PIN);
	
	gpio_init(KEY_OK_PIN, GPI, 1);
	port_pull(KEY_OK_PIN);
	
	gpio_init(KEY_DOWN_PIN, GPI, 1);
	port_pull(KEY_DOWN_PIN);
}
