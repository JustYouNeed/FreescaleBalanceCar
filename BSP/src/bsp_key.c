...
/*  检测按键是否按键的函数  */
static uint8_t IsKeyUpPress(void) { return (gpio_get(KEY_UP_PIN) == 1)?0:1;}
static uint8_t IsKeyOkPress(void) { return (gpio_get(KEY_OK_PIN) == 1)?0:1;}
static uint8_t IsKeyDownPress(void) { return (gpio_get(KEY_DOWN_PIN) == 1)?0:1;}
