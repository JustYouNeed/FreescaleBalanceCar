...
{
//	Car.PID.Turn_Kp = drv_flash_ReadSector(PID_PARA_FLASH_ADDR, 0, float);
//	Car.PID.Turn_Ki = drv_flash_ReadSector(PID_PARA_FLASH_ADDR, 4, float);
//	Car.PID.Turn_Kd = drv_flash_ReadSector(PID_PARA_FLASH_ADDR, 8, float);
//	
...
{	
	...
	FLASH_WriteSector(PID_PARA_FLASH_ADDR, (const uint8_t*)&Car.PID, 48, 0);
}
